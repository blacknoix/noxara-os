//! Request identity extraction for companyos-inventory.
//!
...
/// Authenticated caller for an Inventory request.
#[derive(Debug, Clone)]
pub struct AuthCtx {
...
/// Inventory never mints tokens, so this only needs to *verify* — but a
...
/// `jwks_signing_key` (read-only — Inventory never writes this table). Lets
