//! Request identity extraction for companyos-finance.
//!
...
//! `membership` / `auth_session` tables it needs (never CRM/`sales_*` tables).
//!
//! Also accepted:
...
/// Authenticated caller for a Finance request.
#[derive(Debug, Clone)]
pub struct AuthCtx {
...
/// ephemeral dev secret, matching `companyos-core`'s `build_keyring`). Finance
/// never mints tokens, so this only needs to *verify* — but a shared secret
/// with core is required for tokens minted there to validate here.
...
/// `jwks_signing_key` (read-only — Finance never writes this table). Lets
