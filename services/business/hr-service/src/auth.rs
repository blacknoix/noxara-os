//! Request identity extraction for companyos-hr.
//!
...
/// Authenticated caller for a People request.
#[derive(Debug, Clone)]
pub struct AuthCtx {
...
/// ephemeral dev secret, matching `companyos-core`'s `build_keyring`). People
...
/// `jwks_signing_key` (read-only — People never writes this table). Lets
