//! Request identity extraction for companyos-crm.
//!
...
//! `membership` / `auth_session` tables it needs (never finance tables).
//!
//! Also accepted:
...
/// Authenticated caller for a CRM request.
#[derive(Debug, Clone)]
pub struct AuthCtx {
...
/// ephemeral dev secret, matching `companyos-core`'s `build_keyring`). CRM
...
/// `jwks_signing_key` (read-only — CRM never writes this table). Lets
