//! Request identity extraction for companyos-project.
//!
...
/// ephemeral dev secret, matching `companyos-core`'s `build_keyring`). Operations
...
/// `jwks_signing_key` (read-only — Operations never writes this table). Lets
