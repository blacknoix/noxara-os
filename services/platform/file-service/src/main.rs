...
use companyos_file::{auth, build_router, migrate, state::AppState};
use companyos_telemetry::init_tracing;
use tracing::info;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    init_tracing("companyos-file");
    ...
    let addr: SocketAddr = std::env::var("FILE_BIND")
        .unwrap_or_else(|_| "0.0.0.0:8089".into())
        .parse()?;
    info!(%addr, "companyos-file listening");
