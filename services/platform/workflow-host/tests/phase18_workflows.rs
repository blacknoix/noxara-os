...
fn workflow_id_and_namespace_defaults() {
    ...
    assert_eq!(
        workflow_id("org_abc", WorkflowType::PayrollRun, "payrun_1"),
        "org_abc:PayrollRun:payrun_1"
    );
    assert_eq!(
        workflow_id("org_abc", WorkflowType::UserWorkflow, "wfd_1:wfi_1"),
        "org_abc:UserWorkflow:wfd_1:wfi_1"
    );
    assert_eq!(
        workflow_id("org_abc", WorkflowType::AgentRun, "agrun_1"),
        "org_abc:AgentRun:agrun_1"
    );
    ...
    assert_eq!(WorkflowType::all().len(), 15);
}
