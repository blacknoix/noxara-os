//! CompanyOS Configurable Workflow Engine — Phase 3.1.
//!
//! Standalone network service under `/api/v1/workflows/...` (port 8094).

use std::net::SocketAddr;

use companyos_telemetry::init_tracing;
use companyos_workflow::{auth, build_router, migrate, state::AppState};
use tracing::info;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    init_tracing("companyos-workflow");
    ...
    let addr: SocketAddr = std::env::var("WORKFLOW_BIND")
        .unwrap_or_else(|_| "0.0.0.0:8094".into())
        .parse()?;
    let local = auth::local_auth_enabled();
    info!(%addr, local_auth = local, "companyos-workflow listening");
