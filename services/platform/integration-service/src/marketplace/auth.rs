//! Request identity for session-authenticated marketplace routes.
//!
//! Marketplace endpoints are deliberately **not** on the public API-key
//! allowlist: they accept a session access token (or, locally, the dev headers).

use axum::extract::FromRequestParts;
use axum::http::request::Parts;
use axum::http::HeaderMap;
use companyos_auth_token::{verify_access_token, KeyRing};
...
const DEV_ROLE_HEADER: &str = "x-companyos-dev-role";
...
pub struct AuthCtx {
    ...
    /// True when identity came from the LOCAL-ONLY dev headers; role
    /// permissions are then derived from the header role instead of the DB.
...
pub async fn load_rotated_keys(pool: &sqlx::PgPool, ring: &KeyRing) -> Result<(), sqlx::Error> {
