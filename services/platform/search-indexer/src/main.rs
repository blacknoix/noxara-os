...
use companyos_search::{auth, build_router, migrate, state::AppState};
use companyos_telemetry::init_tracing;
use tracing::info;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    init_tracing("companyos-search");
    ...
    let addr: SocketAddr = std::env::var("SEARCH_BIND")
        .unwrap_or_else(|_| "0.0.0.0:8086".into())
        .parse()?;
    info!(%addr, "companyos-search listening");
