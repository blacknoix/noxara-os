...
CREATE POLICY IF NOT EXISTS search_index_job_tenant_isolation ON search_index_job
    USING (org_id = NULLIF(current_setting('app.org_id', true), '')::uuid)
    WITH CHECK (org_id = NULLIF(current_setting('app.org_id', true), '')::uuid);

CREATE POLICY IF NOT EXISTS search_index_job_ingest ON search_index_job
