...
/// Run authz decide() and return a tool trace entry (allow/deny + reason).
pub fn decide_traced(
    principal: &Principal,
    permission: PermissionId,
    tool_name: &str,
    args_summary: &str,
) -> crate::types::ToolTraceEntry {
    let detail = companyos_authz::decide(principal, &permission);
    crate::types::ToolTraceEntry {
        tool_name: tool_name.to_string(),
        permission: permission.as_str().to_string(),
        decision: if detail.decision == Decision::Allow {
            "allow".into()
        } else {
            "deny".into()
        },
