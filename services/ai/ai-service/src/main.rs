...
use companyos_ai::{auth, build_router, migrate, state::AppState};
use companyos_telemetry::init_tracing;
use tracing::info;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    init_tracing("companyos-ai");
    ...
    let addr: SocketAddr = std::env::var("AI_BIND")
        .unwrap_or_else(|_| "0.0.0.0:8092".into())
        .parse()?;
    info!(%addr, "companyos-ai listening");
