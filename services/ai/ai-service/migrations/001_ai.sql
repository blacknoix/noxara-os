...
CREATE POLICY IF NOT EXISTS ai_org_settings_tenant ON ai_org_settings
...
CREATE POLICY IF NOT EXISTS ai_session_tenant ON ai_session
    USING (org_id = NULLIF(current_setting('app.current_org_id', true), '')::uuid)
    WITH CHECK (org_id = NULLIF(current_setting('app.current_org_id', true), '')::uuid);
...
CREATE POLICY IF NOT EXISTS ai_interaction_tenant ON ai_interaction
    USING (org_id = NULLIF(current_setting('app.current_org_id', true), '')::uuid)
    WITH CHECK (org_id = NULLIF(current_setting('app.current_org_id', true), '')::uuid);
...
CREATE POLICY IF NOT EXISTS ai_proposal_tenant ON ai_proposal
    USING (org_id = NULLIF(current_setting('app.current_org_id', true), '')::uuid)
    WITH CHECK (org_id = NULLIF(current_setting('app.current_org_id', true), '')::uuid);
...
CREATE POLICY IF NOT EXISTS ai_document_review_tenant ON ai_document_review
    USING (org_id = NULLIF(current_setting('app.current_org_id', true), '')::uuid)
    WITH CHECK (org_id = NULLIF(current_setting('app.current_org_id', true), '')::uuid);
