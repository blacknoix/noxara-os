...
CREATE POLICY IF NOT EXISTS hello_tenant_isolation ON hello_message
...
CREATE POLICY IF NOT EXISTS audit_tenant_isolation ON audit_entry
    USING (org_id = NULLIF(current_setting('app.org_id', true), '')::uuid)
    WITH CHECK (org_id = NULLIF(current_setting('app.org_id', true), '')::uuid);
...
CREATE POLICY IF NOT EXISTS outbox_tenant_isolation ON outbox_event
    USING (org_id = NULLIF(current_setting('app.org_id', true), '')::uuid)
    WITH CHECK (org_id = NULLIF(current_setting('app.org_id', true), '')::uuid);
